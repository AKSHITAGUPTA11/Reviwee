...
exports.confirmOnlinePayment = async (req, res) => {
  try {
    ...
    const subscriptionPlan = await getSubscriptionDetailsById(
    ...
    await profileService.updateMany(
      { userId: new mongoose.Types.ObjectId(updatedPackage.customerId) },
      {
        $set: {
     ...
     * create invoice
     */
    let user = await customerService.getOneByMultiField({
      _id: new mongoose.Types.ObjectId(subscription.customerId),
    });
    let invoiceRes = await createInvoiceApi({ subscription, user });
    /* ================= LEDGER ================= */
    const isUpgrade =
    ...
    const redirectResult = await afterLoginRedirectTo(subscription.customerId);
    ...
    console.log("confirmOnlinePayment error:", err);
      ...
      additionalQuery = [
        {
          $match: {
            ...
            customerId: new mongoose.Types.ObjectId(req.userData.Id),
            paymentStatus: transactionStatus.success,
            planStatus: { $ne: planStatusEnum.pending },
          },
        },
      ];
    }
...
exports.getById = async (req, res) => {
  try {
    ...
    let additionalQuery = [
      {
        $match: {
          customerId: new mongoose.Types.ObjectId(idToBeSearch),
          planStatus: planStatusEnum.active,
          isDeleted: false,
        },
      },
