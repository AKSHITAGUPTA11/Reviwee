...
const { searchKeys } = require("./schema.transaction");
...
exports.allFilterPagination = async (req, res) => {
  try {
    ...
    let booleanFields = [];
    let numberFileds = [];
    let objectIdFields = ["customerId", "businessId"];
    ...
    //-----------------------------------
    let dataFound =
      await transactionService.aggregateQuery(finalAggregateQuery);
    ...
    let result = await transactionService.aggregateQuery(finalAggregateQuery);
    if (result.length) {
...
exports.razorpayWebhook = async (req, res) => {
  try {
    ...
    // Razorpay expects 200 always if webhook is received
