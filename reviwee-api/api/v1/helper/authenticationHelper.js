...
const customerSubcriptionService = require("../src/customerSubcription/service.customerSubcription");
const profileService = require("../src/profile/service.profile");
...
const afterLoginRedirectTo = async (usrId) => {
  try {
    let userId = new mongoose.Types.ObjectId(usrId);

    if (
      !(await customerSubcriptionService.getOneByMultiField({
        customerId: userId,
        isActive: true,
        isDeleted: false,
      }))
    ) {
      return { status: true, redirectTo: "SUBSCRIPTION" };
    }

    if (
      !(await profileService.getOneByMultiField({
        userId,
        isActive: true,
        isDeleted: false,
      }))
    ) {
      return { status: true, redirectTo: "PROFILE" };
    }

    return { status: true, redirectTo: "" };
  } catch (err) {
    console.log("afterLoginRedirectTo error", err);
    return { status: false };
  }
};
