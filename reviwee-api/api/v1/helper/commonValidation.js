...
const { variableEnum } = require("../../utils/enumUtils");
...
const googleBusinessLink = (value, helpers) => {
  const regex =
    /^https:\/\/(www\.)?(google\.[a-z.]+\/(maps|search)|maps\.google\.[a-z.]+|g\.page|maps\.app\.goo\.gl)(\/.*)?$/;
...
const allowedVariables = Object.values(variableEnum);

const validateVariables = (value, helpers) => {
  const matches = value.match(/{{(.*?)}}/g) || [];

  for (let variable of matches) {
    if (!allowedVariables.includes(variable)) {
      return helpers.message(`Invalid variable name.`);
    }
  }
