...
type Props = {
  ...
  valueNumber: string;
  placeholder?: string;
  onSelectChange: (selectedOption: Option | any) => void;
  onNumberChange: (e: React.ChangeEvent<HTMLInputElement>) => void;
onNumberBlur?: (event: React.FocusEvent<HTMLInputElement>) => void;

};

const ATMTextFieldWithSwitch: React.FC<Props> = ({
  label = "",
  required = false,
    disabled = false,
  name,
      ...
      <div className={`${label ? "mt-2 " : ""}form-field-focus flex w-full px-2 h-[42px] text-slate-700 border border-[var(--divider)] rounded-[var(--radius-sm)]`}>
        ...
        <div className="w-fit">
          <Select
            ...
            options={options}
            value={valueSelect}
            onChange={(option: any) => onSelectChange(option)}
            isSearchable
...
export default ATMTextFieldWithSwitch;
