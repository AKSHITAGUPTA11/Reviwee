        ...
        {isLoading ? (
          Array(10)
            ...
            .map((_, index) => (
              <React.Fragment key={index}>
                {isCheckbox && (
                  <div className="animate-pulse h-[50px] p-2">
          ...
          rows?.map((row: any, rowIndex) => (
            <React.Fragment key={row[idKey] || rowIndex}>
              {isCheckbox && (
                <div
                  className={`flex items-center px-2 ${row?.isActive ? "bg-white hover:bg-slate-200" : "bg-slate-300"} ${rowIndex === rows?.length - 1 && "rounded-bl"}`}
                  onClick={(e) => e.stopPropagation()}
                >
                  <input
                    type="checkbox"
