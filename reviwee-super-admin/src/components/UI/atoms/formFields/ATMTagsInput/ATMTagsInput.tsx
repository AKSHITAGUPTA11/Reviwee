...
const ATMTagsInput = ({
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
        ...
        if (e.keyCode === 13 && text.trim() && safeTags.length < max) {
            e.preventDefault();
            setTags([...safeTags, text.trim()]);
            setCurrentTagText("");
        } else if (addOnSpace && e.keyCode === 32 && text.trim() && safeTags.length < max) {
            e.preventDefault();
            setTags([...safeTags, text.trim()]);
            setCurrentTagText("");
        }
